...
void main( void ) {
  vec2 position = (gl_FragCoord.xy - resolution.yy * 0.5) / resolution.yy;
  float th = atan(position.y, position.x) / (1.0 * Phi);
  ...
  float a = 0.5 + cos(uv.x * Tau) * 0.5;
  float b = 0.5 + cos(uv.y * Tau) * 0.5;
  float c = 0.5 + cos(uv.z * Tau) * 0.5;
  ...
  gl_FragColor = vec4( (color * dd), Phi);
}
